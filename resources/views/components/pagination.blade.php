@if ($paginator->hasPages())
    <nav class="inline-flex items-center gap-2 mt-4" role="navigation" aria-label="Pagination">
        @if ($paginator->onFirstPage())
            <span class="px-3 py-2 rounded-lg border border-white/10 hover:border-fuchsia-500/40">&laquo;</span>
        @else
        ...
        @foreach($elements as $element)
            ...
            @if (is_array($element))
                @foreach($element as $page => $url)
                    ...
                    @else
                        <a href="{{ $url }}" class="px-3 py-2 rounded-lg bg-fuchsia-500/80 border border-fuchsia-500/30">{{ $page }}</a>
                    @endif
                @endforeach
            @endif
        @endforeach
