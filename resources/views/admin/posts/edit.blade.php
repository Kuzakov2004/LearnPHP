...
<form class="glass rounded-2xl p-6 border border-white/10 space-y-5 flex flex-col gap-3" action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="flex flex-col">
        ...
        <textarea class="input border px-3 py-1 rounded-xl" rows="3" placeholder="Текст для списка постов..." name="excerpt">{{ old('excerpt', $post->excerpt) }}</textarea>
