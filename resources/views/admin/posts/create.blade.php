...
<form class="glass rounded-2xl p-6 border border-white/10 space-y-5 flex flex-col gap-3" action="{{ route('posts.store') }}" method="POST">
    ...
    <div class="flex flex-col">
        ...
        <textarea class="input border px-3 py-1 rounded-xl" rows="3" placeholder="Текст для списка постов..." name="excerpt"></textarea>
        @error('excerpt') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
    </div>

    <div class="flex flex-col">
        <label class="label">Текст</label>
        <textarea class="input border px-3 py-1 rounded-xl" rows="10" placeholder="Основной контент..." name="body"></textarea>
